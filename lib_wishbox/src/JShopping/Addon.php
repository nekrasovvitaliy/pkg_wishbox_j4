...
class Addon extends AddonCore implements AddonInterface
{
	...
	/**
	 * @var string $errorCategory Error category
	 *
	 * @since 1.0.0
	 */
	...
	{
		return $this->addon_alias; // phpcs:ignore
	}
	...
	public function showAdminFormParams(): void
	{
		...
		$pluginName = str_replace('wishbox', 'wishboxadmin', $this->addon_alias);
		$this->language->load(
			'plg_jshoppingadmin_' . $pluginName, JPATH_PLUGINS . '/jshoppingadmin/' . $pluginName
		);
		$this->language->load('plg_jshoppingadmin_' . $pluginName, JPATH_ADMINISTRATOR);
	...
	public function renderSettingsForm(string $addonAlias): string
	{
		...
		$pluginName = str_replace('wishbox', 'wishboxadmin', $addonAlias);
		$this->language->load(
			'plg_jshoppingadmin_' . $pluginName, JPATH_PLUGINS . '/jshoppingadmin/' . $pluginName
		);
		$this->language->load('plg_jshoppingadmin_' . $pluginName, JPATH_ADMINISTRATOR);
